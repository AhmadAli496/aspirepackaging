  ...
  return (
    <div className={'fixed top-0 right-0 left-0 shadow-md z-40 bg-white'}>
      <header className="header flex items-center justify-between max-w-[1440px] px-4 md:px-[1rem] mx-auto py-[1rem]">
        <div>
          <Link href={'/'}><Image src="/logo.png" alt='This is the logo for my website' className={'w-[240px] md:w-[300px] md:h-[47px]'} width={300} height={50} /></Link>
        </div>
        <div className="navlink">
          ...
          {open ? (
            <Image onClick={() => setOpen(false)} src={'/close.png'} alt='close icon' width={30} height={30}/>

          ): (
            <Image onClick={handleClick} src={'/menu.png'} alt={'hamburger menu icon'} className={'lg:hidden'} width={30} height={30} />
          )}
          {open && (
              <div className={'h-screen mt-[4rem] bg-white shadow-2xl backdrop-blur-2xl absolute top-0 left-0 right-0 z-50 lg:hidden'}>
              <nav className="navbar_links  w-full gap-6  flex">
                <ul className="link  mt-4 border-b  gap-12 flex p-6 w-full flex-col text-[18px] md:text-[16px] font-semibold">
